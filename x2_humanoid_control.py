...
q0_end = -np.pi / 5  # desired end joint angle of human knee joint
# q1_init = 0 # Initial joint angle of human hip joint
# q1_end = np.pi /3  # desired end joint angle of human hip joint
...
data.qpos[16] = q0_init
print("************** the init knee joint angle is: ", data.qpos[16], " and ", q0_init)
...
with open("sensor_data.csv", mode="a") as file:
    ...
    while not glfw.window_should_close(window):
        ...
        if data.time >= simend:
            ...
            plt.subplot(3, 1, 2)
            # plt.plot(t, knee_joint_smooth_force, "g-")
            plt.plot(t, knee_joint_bias_force, "b-")
            # plt.plot(t, human_knee_torque, "r")

            # plt.plot(t, np.add(knee_joint_smooth_force, knee_joint_bias_force), "y")
            plt.plot(t, torque_calculated, "k")
            # using np.add because the bias force and torque calculated are in opposite directions
            plt.plot(t, np.add(knee_joint_bias_force, torque_calculated), "y")
            plt.plot(t, knee_passive_force, "r")
            # plt.plot(t, exo_knee_act_force, "g") #actuation force is actually 0 all the time
            # qfrc_smooth = Fbias + Fpassive + Factuation
            sum = np.add(
                knee_joint_bias_force, np.add(knee_passive_force, exo_knee_act_force)
            )
            plt.plot(t, sum, "g-")
            plt.plot(t, knee_joint_smooth_force, "c")
            # plt.plot(t, exo_knee_contraint_force, "m")
            plt.legend(
                [
                    # "knee_joint_smooth_force",
                    "knee_joint_bias_force",
                    # "active torque from human knee",
                    # "net passive force",
                    "grav_torque_calculated",
                    "the difference between the bias force and the gravitational torque",
                    "knee_passive_force",
                    # "knee_actuation_force",
                    "sum of the bias, passive and actuation forces",
                    "knee_smooth_force",
                    # "exo_knee_constraint_force",
                ]
            )
            plt.ylabel("force (N)")
            ...
            plt.plot(t, exo_knee_contraint_force, "b-")
            plt.plot(t, knee_joint_smooth_force, "c")
            plt.plot(t, knee_passive_force, "r")

            plt.legend(
                [
                    "Exo knee constraint force",
                    "knee_smooth_force",
                    "knee_passive_force",
                    # "exo_left_knee_position",
                    # "exo_left_knee_inertia_position",
                ]
            )
